...
## Key Points
- Hewitt and Manning test whether dependency-tree distances and depths correspond to squared L2 distances and norms after a learned linear transformation.
- their rank analysis suggests syntactic structure can be recovered from a lower-dimensional transformed space.
- Jawahar et al. use TPDN approximation to test whether BERT representations are better approximated by tree-like role schemes than by several linear or bag-of-words alternatives.
- geometry-based evidence is more structured than ordinary classification accuracy, but still requires caution about causal use.
