...
# Source - What Does BERT Learn About the Structure of Language
...
## Key Points
- Source fact: the paper studies `bert-base-uncased` across its 12 layers.
- Source fact: lower BERT layers cluster phrase-level span representations better than higher layers in the authors' chunking-based analysis.
- Source fact: using the Conneau et al. probing tasks, the authors report surface features strongest in lower layers, syntactic features strongest in middle layers, and semantic features strongest in higher layers.
- Source fact: subject-verb agreement performance is strongest in middle layers for many cases, with deeper layers helping when long-distance dependencies and more attractors are involved.
- Source fact: using Tensor Product Decomposition Networks, the authors report that tree-based role schemes best approximate BERT representations across many layers.
- Source fact: the paper concludes that BERT captures structural properties of English and a compositional pattern with parallels to traditional syntactic analysis.
