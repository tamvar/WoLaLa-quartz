...
# Source - The Vector Grounding Problem
...
## Key Points
- Source fact: the paper reformulates the symbol grounding problem as the `Vector Grounding Problem` for vector-based models such as LLMs.
- Source fact: it distinguishes multiple notions of grounding and argues that only `referential grounding` is central to the key philosophical issue.
- Source fact: the authors argue that multimodality and embodiment are neither necessary nor sufficient for solving the grounding problem.
- Source fact: they provide two routes by which LLMs may achieve referential grounding:
...
- Source fact: the paper explicitly separates grounding from stronger claims about understanding, agency, or knowledge.
