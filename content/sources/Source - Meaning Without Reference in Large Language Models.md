...
## Key Points
- the paper directly contests the claim that text-only LLMs must lack meaning because they lack reference.
- it argues that many meaningful human concepts either lack stable referents or cannot be reduced to reference alone.
- the paper uses conceptual role theory to argue that meaning depends on structured relations among internal states.
- it argues that the right place to look for meaning in a model is the geometry and dynamics of its internal representations, not only its architecture, training data, or objective.
- it treats current LLMs as partial and imperfect, but still plausibly meaning-bearing in ways that resemble human conceptual organization.
